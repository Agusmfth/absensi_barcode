...
@section('actions')
    @if (auth()->user()->isAdmin())
        <div class="page-actions">
            <a class="btn btn-outline-secondary page-action" href="{{route('students.cards.all',['class_id'=>request('class_id'),'search'=>request('search')])}}" data-skip-loading="true"><i class="bi bi-printer"></i><span>Cetak Semua Kartu</span></a>
            <button class="btn btn-outline-primary page-action" data-bs-toggle="modal" data-bs-target="#importStudentModal" type="button">
                <i class="bi bi-file-earmark-arrow-up"></i><span>Import Excel</span>
            </button>
...
<div class="alert alert-warning"><div class="fw-semibold mb-1"><i class="bi bi-exclamation-triangle"></i> Ada baris yang tidak diimpor</div><ul class="mb-0 ps-3">@foreach(session('import_errors') as $error)<li>{{$error}}</li>@endforeach</ul></div>
@endif
@if($errors->has('student_file'))<div class="alert alert-danger">{{$errors->first('student_file')}}</div>@endif
